...
const useFetchFideicomisarioPInfo = (numeroDeContrato, numeroDeFideicomisario) => {
	const [data, setData] = useState(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState(null);
	const { config } = useFetchConfig();
	useEffect(() => {
		if(!numeroDeContrato&&!numeroDeFideicomisario||!config){
			setLoading(true);
			return;
		}
		const fetchData = async () => {
			...
			try {
				...
				const response = await fetch(`${uriApi}/api/fideicomisariop/info/${numeroDeContrato}/fidsario/${numeroDeFideicomisario}`, {
					method: 'GET',
					headers: headers,
				});
		...
		fetchData();
	}, [numeroDeContrato, numeroDeFideicomisario, config]);

	return { data, loading, error };

};

export default useFetchFideicomisarioPInfo;