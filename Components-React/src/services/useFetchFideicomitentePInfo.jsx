...
const useFetchFideicomitentePInfo = (numeroDeContrato, numeroDeFideicomitente) => {
	const [data, setData] = useState(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState(null);
	const { config } = useFetchConfig();
	useEffect(() => {
		if(!numeroDeContrato&&!numeroDeFideicomitente||!config){
			setLoading(true);
			return;
		}
		const fetchData = async () => {
			...
			try {
				...
				const response = await fetch(`${uriApi}/api/fideicomitentep/info/${numeroDeContrato}/fideicom/${numeroDeFideicomitente}`, {
					method: 'GET',
					headers: headers,
				});
		...
		fetchData();
	}, [numeroDeContrato, numeroDeFideicomitente, config]);

	return { data, loading, error };

};

export default useFetchFideicomitentePInfo;