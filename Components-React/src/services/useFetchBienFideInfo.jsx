...
const useFetchBienFideInfo = (idBien, numeroDeContrato, numeroDeSubcontrato) => {
	const [data, setData] = useState(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState(null);
	const { config } = useFetchConfig();
	useEffect(() => {
		if(!idBien&&!numeroDeContrato&&!numeroDeSubcontrato||!config){
			setLoading(true);
			return;
		}
		const fetchData = async () => {
			...
			try {
				...
				const response = await fetch(`${uriApi}/api/bienfide/info/${idBien}/contrato/${numeroDeContrato}/subcontrato/${numeroDeSubcontrato}`, {
					method: 'GET',
					headers: headers,
				});
		...
		fetchData();
	}, [idBien, numeroDeContrato, numeroDeSubcontrato, config]);

	return { data, loading, error };

};

export default useFetchBienFideInfo;