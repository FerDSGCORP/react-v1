...
const useFetchTerceroPInfo = (numeroDeContrato, numeroDeTercero) => {
	const [data, setData] = useState(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState(null);
	const { config } = useFetchConfig();
	useEffect(() => {
		if(!numeroDeContrato&&!numeroDeTercero||!config){
			setLoading(true);
			return;
		}
		const fetchData = async () => {
			...
			try {
				...
				const response = await fetch(`${uriApi}/api/tercerop/info/${numeroDeContrato}/tercero/${numeroDeTercero}`, {
					method: 'GET',
					headers: headers,
				});
		...
		fetchData();
	}, [numeroDeContrato, numeroDeTercero, config]);

	return { data, loading, error };

};

export default useFetchTerceroPInfo;