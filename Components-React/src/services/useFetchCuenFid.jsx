...
const useFetchCuenFid = (page = 1, rows = 80, filters = null) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				// Si se reciben filtros, los convertimos en JSON y los a�adimos al header
				if (filters) {
				...
				const response = await fetch(
					`${uriApi}/api/CuenFid/sidx/NumeroDeContrato/sord/asc/page/${page}/rows/${rows}`,
					{
						method: 'GET',
						 headers: headers,
					}
